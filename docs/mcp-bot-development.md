...
> 版本：对应 device-manager `1.0.3`；MCP 协议目标 **`2026-07-28`（Modern，无状态）**。
...
```json
{
  ...
  "result": {
    ...
    "_meta": {
      "io.modelcontextprotocol/serverInfo": { "name": "device-manager", "version": "1.0.3" }
    },
...
- `bot` job：`actions/setup-node@v4`（`node-version: 22`、`cache: pnpm`）→ `corepack enable` → `pnpm install --frozen-lockfile` → `pnpm run typecheck`（`tsc --noEmit`）→ `pnpm test`（vitest）→ buildx 推 GHCR + Docker Hub `device-manager-bot`（`latest-bot` / `1.0.3-bot`，与核心 tag 对齐）。
