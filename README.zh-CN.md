...
| 🚀 版本更新 | 定时对比 GitHub Releases 检测新版本并在界面提示；管理员可开启「直接更新」下载产物替换自身（看门狗脚本负责拉起） |
...
- **构建与部署**：Maven；GitHub Actions 在每次 `v*` 标签上构建可执行 jar 并附加到 GitHub Release。
...
| `app.update.asset-prefix` | Release 产物文件名前缀，默认 `device-manager`（即 `device-manager.jar`） |
...
| `JWT_SECRET` | JWT 签名密钥，覆盖 `application.yaml` 里的 `jwt.secret` | 见 `jwt.secret` |
| `TZ` | 时区，如 `Asia/Shanghai` | 系统默认 |
...
服务启动后会定时（默认 6 小时）调用 GitHub Releases API 对比当前版本与最新版本，有新版本时顶栏出现提示。当前版本由构建时注入（`app.version`，即 `pom.xml` 的版本号），因此正式发布的版本号与 Release 标签一致。

管理员可在弹窗里开启「直接更新」。开启后应用会下载 Release 产物（`device-manager.jar`）替换自身并退出，再由**看门狗脚本**拉起新版本。看门狗脚本可直接在弹窗里下载：
...
> ⚠️ 直接更新会替换正在运行的 jar 并退出进程，**必须**由看门狗或 systemd 之类的守护进程拉起，否则服务不会自动恢复。
